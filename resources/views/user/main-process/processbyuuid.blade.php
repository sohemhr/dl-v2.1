...
@section('content')
    ...
    <section class="content">
                ...
                <div class="col-md-12">
                    ...
                    <div class="card card-info">
                        ...
                        @foreach ($getProcess as $val)
                            ...
                            <form class="form-horizontal" action="/admstr/main-process/onprocess/update/{{ $val->trxdtl_uuid }}"
                                ...
                                <div class="card-body">
                                    <div class="form-group row">
                                        ...
                                        <div class="col-sm-10">
                                            <input type="text" name="layanan_nama"
                                                class="form-control @error('layanan_nama') is-invalid @enderror"
                                                id="layanan_nama" placeholder="Nama Layanan"
                                                value="{{ $val->layanan_kode . ' - ' . $val->layanan_nama }}" readonly>
                                            @error('layanan_nama')<span id="layanan_nama"
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-sm-10">
                                            <input type="text" name="jenis_nama"
                                                class="form-control @error('jenis_nama') is-invalid @enderror" id="jenis_nama"
                                                placeholder="Jenis Layanan"
                                                value="{{ $val->jenis_kode . ' - ' . $val->jenis_nama }}" readonly>
                                            @error('jenis_nama')<span id="jenis_nama"
                                            class="error invalid-feedback">{{ $message }}</span>@enderror
                                        </div>
                                    </div>
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-sm-10">
                                            <textarea name="trxdtl_notes"
                                                class="form-control @error('trxdtl_notes') is-invalid @enderror"
                                                id="trxdtl_notes" placeholder="Keterangan untuk proses ini.."
                                                rows="3">{{ $val->trxdtl_notes }}</textarea>
                                            @error('trxdtl_notes')<span id="trxdtl_notes"
                                            class="error invalid-feedback">{{ $message }}</span>@enderror
                                        </div>
                                    </div>
                                ...
                                <div class="card-footer text-center">
                                    ...
                                    <button type="submit" class="btn btn-info float-right">Submit Notes & Status</button>
                                </div>
                                <!-- /.card-footer -->
                            </form>
                        @endforeach
                    ...
                    <div class="card card-info">
                        ...
                        <div class="card-body">
                            <div class="row mb-4">
                                <a href="javascript:void(0)" id="createNewStatus">
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <button type="button" class="btn btn-xs btn-success" data-toggle="modal"
                                                    data-target="#modal-edit{{ $sts->trxdtlsts_kode }}">
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form id="statusForm" name="statusForm" class="form-horizontal">
                                @csrf
                                <input type="text" name="trxdtl_kode" id="trxdtl_kode" value="{{ $val->trxdtl_kode }}" hidden>
                                <div class="row">
                                    <div class="col-md-2">
                                        <label for="trxdtlsts_nama" class="control-label">
                                            New Process
                                        </label>
                                    </div>
                                    <div class="col-md-10 mb-4">
                                        <select class="form-control select2" id="trxdtlsts_nama" name="trxdtlsts_nama"
                                            required="">
                                            @php
                                                $noList = 1;
                                            @endphp
                                            @foreach ($getListProcess as $inList)
                                                <option value="{{ $inList->lp_kode }}">{{ $noList++ . '. ' . $inList->lp_nama }}
                                                </option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-md-2">
                                        <label for="trxdtlsts_keterangan" class="control-label">
                                            Keterangan
                                        </label>
                                    </div>
                                    <div class="col-md-10">
                                        <textarea name="trxdtlsts_keterangan" id="trxdtlsts_keterangan" class="form-control"
                                            rows="3"></textarea>
                                    </div>
                                </div>
        ...
        {{-- new test update --}}
        @foreach ($listStsDtl as $key)
            <div class="modal fade" id="modal-edit{{ $key->trxdtlsts_kode }}" aria-hidden="true">
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form
                                action="/admstr/main-proces/onprocess/updatedetailprocess/status/{{ $key->trxdtlsts_dtl_kode }}"
                                id="editForm" name="editForm" class="form-horizontal" method="POST">
                                @csrf
                                <input type="text" name="trxdtlsts_kode" id="trxdtlsts_kode" value="{{ $key->trxdtlsts_kode }}"
                                    hidden>
                                <div class="row">
                                    <div class="col-md-2">
                                        <label for="trxdtlsts_nama" class="control-label">
                                    ...
                                    <div class="col-md-10 mb-4">
                                        <select class="form-control select2" id="trxdtlsts_nama" name="trxdtlsts_nama"
                                            required="">
                                            <option value="{{ $key->trxdtlsts_nama }}">*{{ $key->lp_nama }}</option>
                                            @php
                                                $noList = 1;
                                            @endphp
                                            @foreach ($getListProcess as $inList)
                                    ...
                                    <div class="col-md-10">
                                        <textarea name="trxdtlsts_keterangan" id="trxdtlsts_keterangan" class="form-control"
                                            rows="3">{{ $key->trxdtlsts_keterangan }}</textarea>
                                    </div>
                                    <div class="col-md-2 mt-4">
                                        <label for="trxdtlsts_status" class="control-label">
                                            Status
                                        </label>
                                    </div>
                                    <div class="col-md-10 mt-4">
                                        <select class="col-md-8 custom-select" name="trxdtlsts_status">
                                            <option value="{{ $key->trxdtlsts_status }}">*{{ $key->trxdtlsts_status }}</option>
                                            <option value="Start"> Start</option>
                                            <option value="OnProcess"> OnProcess</option>
                                ...
                                <div class="col-sm-12 mt-4">
                                    <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                                    <button type="submit" class="btn btn-primary  float-right" id="btn-update"
                                        value="create">Update
                                    </button>
                                </div>
                            </form>
...
<script>
    $(function () {
        ...
        $('#createNewStatus').click(function () {
            ...
            $('#modelHeading').html("Input New Proses");
        ...
        $('#saveBtn').click(function (e) {
            ...
            $.ajax({
                ...
                url: "{{ route('mainprocess.creatests') }}",
                type: "POST",
                dataType: 'json',
