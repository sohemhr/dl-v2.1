...
<head>
    ...
    <title>Dokter Legal | Reset Password</title>
...
<body>
    ...
    <section class="stark-login">
        ...
        <form action="/my-forgot-password/submit-new-password/{{ $uuid }}" method="post">
            @csrf
            <div id="fade-box">
                @if (session('failed'))
                    <h2>{{ session('failed') }}</h2>
                @endif
                <h2 class="fs-12px">Insert New Password</h2>
                ...
                <p class="fs-14">Login..? <a href="/auth_admstr">Click Here..</a></p>
            </div>
        </form>
        <div class="hexagons">
